...
describe("DefaultTransport — origin validation", () => {
  ...
  it("enforces an explicitly configured allowedOrigin from the very first message", () => {
    const transport = new DefaultTransport({
      allowedOrigin: "https://shell.example.gov",
    });
    const received: unknown[] = [];
    transport.start((message) => received.push(message));

    const fromWrongOrigin = createMessage(
      "handshake",
      "handshake",
      "connect",
      HOST_TARGET,
    ...
    const fromRightOrigin = createMessage(
      "handshake",
      "handshake",
      "connect",
      HOST_TARGET,
  ...
  it("sends outbound messages directly to the pinned origin once one is known", () => {
    ...
    dispatchMessageFromOrigin(
      "https://shell.example.gov",
      createMessage("handshake", "handshake", "connect", HOST_TARGET, "my-app"),
    );
