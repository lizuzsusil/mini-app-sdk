...
function makeModule(
  ...
  const rpc = {
    request: vi.fn(async (_ns: string, action: string) => {
      ...
      if (action === "getTheme")
        return overrides?.theme ?? { preference: "light", mode: "light" };
      return undefined;
    }),
...
describe("appearance module", () => {
  ...
  it("seeds the store from a structured hint (the web shell) without re-deriving", () => {
    ...
    expect(handle.module.state().theme).toEqual({
      preference: "system",
      mode: "dark",
    });
  });
  ...
  it("serves the store instead of an RPC when the host lacks the appearance namespace", async () => {
    ...
    await expect(handle.module.getTheme()).resolves.toEqual({
      preference: "dark",
      mode: "dark",
    });
...
describe("appearance normalizers", () => {
  it("expands locale tags, deriving direction and region", () => {
    expect(normalizeLocale("en")).toEqual({
      locale: "en",
      language: "en",
      direction: "ltr",
    });
  ...
  it("unwraps event-payload wrappers", () => {
    expect(
      normalizeLocale({
        locale: { locale: "si-LK", language: "si", direction: "ltr" },
      }),
    ).toEqual({
      locale: "si-LK",
      language: "si",
    ...
    expect(normalizeTheme({ theme: "dark" })).toEqual({
      preference: "dark",
      mode: "dark",
    });
  });
  ...
  it("resolves `system` against the supplied fallback mode", () => {
    expect(normalizeTheme("system", "dark")).toEqual({
      preference: "system",
      mode: "dark",
    });
    expect(
      normalizeTheme({ preference: "system", mode: "light" }, "dark"),
