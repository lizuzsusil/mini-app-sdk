...
function makeModule(reply: unknown = undefined) {
  const request = vi.fn(async (_ns: string, action: string) => {
    if (action === ACTIONS.NAVIGATION.GET_CURRENT) return { current: '/user/profile', history: ['/user', '/user/profile'] };
    return reply;
  });
...
describe('navigation router', () => {
  it('sends the consumed flag on back', async () => {
    ...
    expect(request).toHaveBeenCalledWith(NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.BACK, { consumed: true });
  });
  ...
  it('sends the consumed flag on push', async () => {
    ...
    expect(request).toHaveBeenCalledWith(NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.PUSH, { consumed: false });
  });
  ...
  it('defaults the flag to true when the caller omits it', async () => {
    ...
    expect(request).toHaveBeenNthCalledWith(1, NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.BACK, { consumed: true });
    expect(request).toHaveBeenNthCalledWith(2, NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.PUSH, { consumed: true });
  });

  it("returns the host's answer when it disagrees with what was requested", async () => {
    const { module } = makeModule({ consumed: false });

    await expect(module.router.back(true)).resolves.toEqual({ consumed: false });
  });
  ...
  it('falls back to the requested flag when the host acknowledges with no payload', async () => {
    ...
    await expect(module.router.back(false)).resolves.toEqual({ consumed: false });
  ...
  it('drives the native-back flow: pops while history remains, stands down at the root', async () => {
    ...
    const onBackRequested = async (module: ReturnType<typeof makeModule>['module']) => {
      const { history } = await module.getCurrent();
      return module.router.back(history.length > 1);
    };

    const deep = makeModule({ consumed: true });
    await expect(onBackRequested(deep.module)).resolves.toEqual({ consumed: true });
    expect(deep.request).toHaveBeenLastCalledWith(NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.BACK, { consumed: true });

    const atRoot = makeModule({ consumed: false });
    atRoot.request.mockImplementation(async (_ns: string, action: string) => {
      if (action === ACTIONS.NAVIGATION.GET_CURRENT) return { current: '/user', history: ['/user'] };
      return { consumed: false };
    });

    await expect(onBackRequested(atRoot.module)).resolves.toEqual({ consumed: false });
    expect(atRoot.request).toHaveBeenLastCalledWith(NAMESPACES.NAVIGATION, ACTIONS.NAVIGATION.BACK, { consumed: false });
  });
